...
# Your personal email for free lifetime admin access
MY_ADMIN_EMAIL = 'boukhdidsalah14@gmail.com'
...
class User(UserMixin, db.Model):
    ...
    def has_active_subscription(self):
        if self.email and self.email.strip().lower() == MY_ADMIN_EMAIL:
            return True
...
with app.app_context():
    ...
    my_user = User.query.filter_by(email=MY_ADMIN_EMAIL).first()
    if my_user:
        if not my_user.is_admin:
            my_user.is_admin = True
            my_user.active_plan = 'Lifetime Admin'
            db.session.commit()
...
/* Side Candlesticks Animation Styles */
.candle-sidebar{position:fixed;top:60px;bottom:0;width:50px;display:flex;flex-direction:column;justify-content:space-around;align-items:center;z-index:2;pointer-events:none;opacity:0.45;overflow:hidden}
.candle-sidebar.left{left:5px}
.candle-sidebar.right{right:5px}
.candlestick{display:flex;flex-direction:column;align-items:center;position:relative;animation:candleFloat 3s infinite ease-in-out alternate}
.candlestick .wick{width:2px;height:35px;background:currentColor}
.candlestick .body{width:10px;height:20px;border-radius:2px;margin:-5px 0}
.candlestick.green{color:var(--green)}
.candlestick.green .body{background:var(--green);box-shadow:0 0 10px var(--green)}
.candlestick.red{color:var(--red)}
.candlestick.red .body{background:var(--red);box-shadow:0 0 10px var(--red)}
@keyframes candleFloat{
  0%{transform:translateY(0) scaleY(0.9);opacity:0.3}
  50%{transform:translateY(-8px) scaleY(1.15);opacity:0.7}
  100%{transform:translateY(8px) scaleY(0.95);opacity:0.4}
}
@media(max-width:1150px){.candle-sidebar{display:none}}
...
# Left and Right animated green/red candlestick sidebars injected into the landing page
CANDLE_SIDEBARS = '''
<div class="candle-sidebar left">
  <div class="candlestick green" style="animation-delay: 0.1s"><div class="wick"></div><div class="body"></div><div class="wick"></div></div>
  <div class="candlestick red" style="animation-delay: 0.5s"><div class="wick"></div><div class="body"></div><div class="wick"></div></div>
  <div class="candlestick green" style="animation-delay: 0.9s"><div class="wick"></div><div class="body"></div><div class="wick"></div></div>
  <div class="candlestick green" style="animation-delay: 0.3s"><div class="wick"></div><div class="body"></div><div class="wick"></div></div>
  <div class="candlestick red" style="animation-delay: 0.7s"><div class="wick"></div><div class="body"></div><div class="wick"></div></div>
  <div class="candlestick green" style="animation-delay: 0.2s"><div class="wick"></div><div class="body"></div><div class="wick"></div></div>
</div>
<div class="candle-sidebar right">
  <div class="candlestick red" style="animation-delay: 0.4s"><div class="wick"></div><div class="body"></div><div class="wick"></div></div>
  <div class="candlestick green" style="animation-delay: 0.8s"><div class="wick"></div><div class="body"></div><div class="wick"></div></div>
  <div class="candlestick red" style="animation-delay: 0.2s"><div class="wick"></div><div class="body"></div><div class="wick"></div></div>
  <div class="candlestick green" style="animation-delay: 0.6s"><div class="wick"></div><div class="body"></div><div class="wick"></div></div>
  <div class="candlestick red" style="animation-delay: 0.9s"><div class="wick"></div><div class="body"></div><div class="wick"></div></div>
  <div class="candlestick green" style="animation-delay: 0.1s"><div class="wick"></div><div class="body"></div><div class="wick"></div></div>
</div>
'''

LANDING = BASE_STYLE + BACKGROUND + CANDLE_SIDEBARS + '''</head><body>{{header|safe}}<div class="wrap">
...
def register():
    ...
    if request.method=='POST':
        ...
        is_admin_flag = (email == MY_ADMIN_EMAIL)
        user=User(email=email, active_plan='Lifetime Admin' if is_admin_flag else 'None', is_admin=is_admin_flag)
        user.set_password(password)
        db.session.add(user)
        db.session.commit()
        login_user(user)
        return redirect(url_for('terminal' if user.has_active_subscription() else 'index'))
...
def login():
    ...
    if request.method=='POST':
        ...
        if user and user.check_password(password): 
            if email == MY_ADMIN_EMAIL and not user.is_admin:
                user.is_admin = True
                user.active_plan = 'Lifetime Admin'
                db.session.commit()
            login_user(user)
            return redirect(url_for('terminal' if user.has_active_subscription() else 'index'))
...
def checkout(plan_name):
    ...
    if current_user.has_active_subscription(): return render_page(CHECKOUT,plan_name=plan_name,plan=PLANS[plan_name],payment=None)
    if request.method=='POST':
        try:
