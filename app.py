...
def migrate_sqlite_schema():
    """Create missing tables/columns without deleting existing users or payments."""
    db.create_all()
    ...
    # Clear any SQLAlchemy objects left in the session by schema changes.
    db.session.remove()
...
threading.Thread(target=run_market_analyzer, daemon=True).start()
...
def render_page(template, **kwargs):
    ...
    signals = Signal.query.order_by(Signal.id.desc()).limit(20).all()
    return render_template_string(template, header_rendered=header_rendered,
        ...
        selected_plan=session.get("selected_plan", "1 Month"), signals=signals, plans=PLANS, **kwargs)
